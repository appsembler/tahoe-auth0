"""
Tests copied from
https://github.com/mdn/django-locallibrary-tutorial/blob/master/catalog/tests/test_views.py
"""
import datetime
import uuid
...
class AuthorListViewTest(TestCase):
    ...
    def test_view_uses_correct_template(self):
        response = self.client.get(reverse("authors"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "demoapp/author_list.html")

    def test_pagination_is_hundred(self):
        response = self.client.get(reverse("authors"))
        self.assertEqual(response.status_code, 200)
        self.assertTrue("is_paginated" in response.context)
        self.assertTrue(response.context["is_paginated"])
        self.assertEqual(len(response.context["author_list"]), 100)
    ...
    def test_lists_all_authors(self):
        ...
        self.assertEqual(len(response.context["author_list"]), 30)
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_logged_in_uses_correct_template(self):
        ...
        self.assertTemplateUsed(
            response, "demoapp/bookinstance_list_borrowed_user.html"
        )
    ...
    def test_pages_paginated_to_hundred(self):

        # Change all books to be on loan.
        # This should make 210 test user ones.
        ...
        # Confirm that only all 100 items are displayed due to pagination
        self.assertEqual(len(response.context["bookinstance_list"]), 100)
    ...
    def test_pages_ordered_by_due_date(self):
        ...
        # Confirm that of the items, only 100 are displayed due to pagination.
        self.assertEqual(len(response.context["bookinstance_list"]), 100)
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_logged_in_with_permission_borrowed_book(self):
        ...
        # Check that it lets us login - this is our book and we
    ...
    def test_logged_in_with_permission_another_users_borrowed_book(self):
        ...
        # Check that it lets us login. We're a librarian, so we can
        # view any users book
        self.assertEqual(response.status_code, 200)
    ...
    def test_uses_correct_template(self):
        ...
        # Check we used correct template
        self.assertTemplateUsed(response, "demoapp/book_renew_librarian.html")
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_uses_correct_template(self):
        ...
        response = self.client.get(reverse("author-create"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "demoapp/author_form.html")
