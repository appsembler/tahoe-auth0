...
from tahoe_idp.models import MagicLink, MagicLinkError, MagicLinkUnsubscribe
...
def test_generate_url(settings, magic_link):  # NOQA: F811
    ...
    ml = magic_link(request)
    url = 'http://{host}{login_url}?token={token}&email={email}'.format(
        host=host,
        login_url=login_url,
        token=ml.token,
        email=quote(ml.email),
    )
...
def test_send_email(mocker, settings, magic_link):  # NOQA: F811
    ...
    send_mail = mocker.patch('tahoe_idp.models.send_mail')
    render_to_string = mocker.patch('tahoe_idp.models.render_to_string')
