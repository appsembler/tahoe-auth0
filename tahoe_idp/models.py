...
class MagicLink(models.Model):
    ...
    def generate_url(self, request: HttpRequest) -> str:
        ...
        url = urljoin('{scheme}://{domain}'.format(scheme=scheme, domain=domain), url_path)
        return url

    def send(self, request: HttpRequest) -> None:
        user = User.objects.get(email=self.email)

        if not magiclink_settings.IGNORE_UNSUBSCRIBE_IF_USER:
            try:
        ...
        context = {
            'subject': magiclink_settings.EMAIL_SUBJECT,
            'user': user,
            'magiclink': self.generate_url(request),
            'expiry': self.expiry,
            'ip_address': self.ip_address,
            'created': self.created,
            'require_same_ip': magiclink_settings.REQUIRE_SAME_IP,
            'require_same_browser': magiclink_settings.REQUIRE_SAME_BROWSER,
            'token_uses': magiclink_settings.TOKEN_USES,
            'style': magiclink_settings.EMAIL_STYLES,
        }
        plain = render_to_string(magiclink_settings.EMAIL_TEMPLATE_NAME_TEXT, context)
        html = render_to_string(magiclink_settings.EMAIL_TEMPLATE_NAME_HTML, context)
        send_mail(
            subject=magiclink_settings.EMAIL_SUBJECT,
            message=plain,
