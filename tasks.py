...
def coverage(c):
    ...
    """
    c.run("coverage run --source tahoe-auth0 runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/tahoe-auth0.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ tahoe_auth0")
...
def lint(c):
    ...
    """
    c.run("flake8 tahoe-auth0 tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=tahoe_auth0.__version__))
    c.run("git push --tags")
